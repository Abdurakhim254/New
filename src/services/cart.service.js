...
export const createcart = async ({
    ...
    if (!created_at || !updated_at) {
        ...
        var query =
            'Insert into cart (user_id,total,created_at,updated_at) Values($1,$2,$3,$4) returning *'
...
export const updatecart = async ({
    ...
    if (!created_at || !updated_at) {
        const query =
            'Update cart set user_id=$1,total=$2 where id=$3 returning *'
        const arr = [user_id, total, id]
        const result = await pool.query(query, arr)
        return result.rows
    } else {
        const query =
            'Update cart set user_id=$1,total=$2,created_at=$3,updated_at=$4 where id=$5 returning *'
