...
export const createcart_item = async ({
    cart_id,
    product_id,
    quantity,
    created_at,
    ...
    if (!created_at || !updated_at) {
        ...
        var query =
            'Insert into cart (cart_id,total,created_at,updated_at) Values($1,$2,$3,$4) returning *'
...
export const updatecart_item = async ({
    cart_id,
    product_id,
    quantity,
    created_at,
    ...
    if (!created_at || !updated_at) {
        const query =
            'Update cart_item set cart_id=$1,product_id=$2,quantity=$3 where id=$4 returning *'
        const arr = [cart_id, product_id, quantity, id]
        const result = await pool.query(query, arr)
        return result.rows
    } else {
        const query =
            'Update cart_item set cart_id=$1,product_id=$2,quantity=$3,created_at=$4,updated_at=$5 where id=$6 returning *'
